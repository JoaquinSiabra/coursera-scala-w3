...
abstract class TweetSet {
  ...
  def filter(p: Tweet => Boolean): TweetSet = 
    filterAcc((tweet)=> p(tweet),new Empty)
   ...
   * and be implemented in the subclasses? ---> It should be implemented in the 
   *  subclasses, because we need to access to "this", and here the class is abstract
   */
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def union(that: TweetSet): TweetSet = (left union (right union that)).incl(elem)
    //this.remove(elem) union (that.incl(elem))  MAL!! ENTRABA EN BUCLE INFINITO
  ...
  def mostRetweeted: Tweet = {
        ...
        if (!left.empty && elem.retweets<leftmost.retweets) 
